...
for (i in seq(0, length(file_list), 1)) {
  print(file_list[i])
  df_to_bind = read.csv(file_list[i]) %>%
    mutate(source_file = file_list[i],
           network_size = network_groups * 4,
           session_size = network_size * total_networks,
           neighbours = case_when(treatment_ref == "A" ~ 2,
                                  TRUE ~ 3),
           round_number = Step,
           all_known = ifelse(agents_known == session_size - 1, round_number, NA),
           all_played = ifelse(agents_played == session_size - 1, round_number, NA))
  if (i == 0) {
    all_abm_df = df_to_bind
  } else {
    all_abm_df = bind_rows(all_abm_df, df_to_bind)
  }
}
...
treat_a_df <- ses_abm_df %>% filter(treatment_ref == "A")
treat_b_df <- ses_abm_df %>% filter(treatment_ref == "B")
treat_c_df <- ses_abm_df %>% filter(treatment_ref == "C")

convergence_a <- ggplot(data = treat_a_df, aes(x = round_number)) +
  geom_line(aes(y = session_consensus_convergence, color = as_factor(total_networks))) +
